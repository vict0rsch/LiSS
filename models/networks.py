...
def define_D(
    ...
    elif netD == "rotational":  # classify if each pixel is real or fake
        net = RotationalNLayerDiscriminator(
...
class RotationalNLayerDiscriminator(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def forward(self, input=None, rotation=None):
        """Standard forward."""
        assert input is not None or rotation is not None
        g = r = None
        if input is not None:
            g = self.gan(self.shared(input))
        if rotation is not None:
            r = self.rotation(self.shared(rotation))
        return g, r
